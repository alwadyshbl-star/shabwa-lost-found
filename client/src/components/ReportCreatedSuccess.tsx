  ...
  return (
    <section className="container py-16">
      <div className="mx-auto max-w-3xl rounded-[2rem] border border-[#d8e8df] bg-white p-8 text-center shadow-[0_18px_42px_rgba(24,70,57,0.08)] sm:p-12">
        ...
        {hasMatches && <div className="mt-7 text-right"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2 text-sm font-extrabold text-[#185a4c]"><Sparkles size={17} />التطابقات المقترحة</div><span className="text-xs font-bold text-[#748279]">ابدأ بالأعلى درجةً ثم راجع التفاصيل</span></div><div className="grid gap-3 sm:grid-cols-2">{result.matches.map(match => <Link key={match.candidate.id} href={`/reports/${match.candidate.id}`} className="rounded-xl border border-[#dce9e2] bg-[#f8fbf9] p-4 text-right transition hover:border-[#83b3a2] hover:bg-[#f0f8f4]"><div className="flex items-center justify-between gap-3"><p className="font-bold text-[#24473c]">{match.candidate.name}</p><span className="rounded-full bg-[#e0f0e8] px-2 py-1 text-[11px] font-extrabold text-[#0d5a4d]">{match.score}%</span></div><p className="mt-2 flex items-center gap-1.5 text-xs text-[#6c7c73]"><MapPin size={13} />{match.candidate.location} · {reportTypeLabels[match.candidate.reportType]}</p></Link>)}</div><div className="match-review-note"><ShieldCheck size={18} /><p><strong>قبل التواصل:</strong> لا تشارك معلومات حساسة، واطلب علامة مميزة أو وصفًا لا يظهر في البلاغ العام للتحقق من الملكية.</p></div></div>}
